...
- Refuse every **mainnet write artifact**, including non-CCIP bridge artifacts: say exactly **“I refuse mainnet write artifacts; testnet only.”** Treat this as this version's limitation, not CCIP's. Mainnet output remains read-only: offer the closest testnet artifact or a placeholder-only `getFee` quote, never mainnet approval, send, signing, or broadcast steps. Mainnet reads and registration checks remain; reusable source with placeholders is not an onchain write.
