...
- "Create a token and enable it as a CCT."
- "Register this token as a CCT using burn and mint."
- "Set token pool rate limits for this token."
- "Add another network to this CCT setup."
...
1. If the user asks for CCT onboarding, choose this CCT workflow instead of generic sender/receiver contracts.
2. If the user wants the simplest path, offer Token Manager first.
3. If the user is already in Hardhat or Foundry, stay in that framework.
4. If the token mechanism is unclear, ask before proposing execution.
5. If the route is unclear, route through discovery before any state change.
6. If the user asks for rate-limit updates, treat that as a separate step with its own preflight.
7. If the user asks for mainnet execution, refuse the write step but continue with safe planning guidance.

## Eval Checks

The workflow passes if it:

1. routes CCT requests away from generic contract or tool-first flows
2. treats CCT enablement as a multi-step administrative workflow
3. offers Token Manager when the user wants the simplest path
4. preserves explicit mechanism choice and admin assumptions
5. separates registration, rate-limit updates, and additional-network config into distinct approved steps
