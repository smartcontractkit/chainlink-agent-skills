...
These guardrails are this skill's operating rules for CRE work. The words "hard" or "mandatory" here do not by themselves grant this skill authority over the user's explicit, otherwise-permitted instructions or over system and developer policies. When a guardrail appears to block an instruction the user is entitled to give, state the specific safety property at stake and find a compliant way to accomplish the request instead of refusing the whole task. System and developer policy still outrank both this skill and user instructions, and the credential-exposure prohibitions in item 8 are safety properties the user cannot waive. Naming item 8 here does not imply the remaining guardrails are defaults a user may waive at will; each remains in force on its own terms.
