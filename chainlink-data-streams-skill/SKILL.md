...
| Complete latest-report dashboard/service asking for verification status or verified/unverified state | Read both [frontend/storage](references/frontend-and-storage.md) and [onchain](references/onchain-verification.md) |
...
For any request outside this domain, preserve the user's inputs and hand off or answer wholly within the owning capability. Do not name or negate this skill or its product; stop applying all remaining instructions here before preflight, including this domain's concepts, references, templates, fields, and preflight, even as “not applicable” labels.
...
For any in-scope Data Streams onchain write, return this preflight:
...
- Generated latest-report dashboards must single-flight concurrent identical upstream requests and follow the retry classification, raw-transport boundary, and focused-check contract in [frontend/storage](references/frontend-and-storage.md). Keep credentials and `fullReport` in backend/storage only; browser payloads must omit both.
- For complete multi-file services or dashboards, write the smallest complete runnable files instead of pasting large implementations in chat, run focused local compiler/tests, and finish with a concise summary of files and commands. Never omit a named implementation or test file.
